...
<head>
  ...
  <style>
    <%@include file="/css/calendarstyle.css" %>
  </style>
    ...
    document.addEventListener('DOMContentLoaded', function(){
      ...
      var calendar = new FullCalendar.Calendar(calendarE1, {
        ...
        <%
          ...
          sql = "SELECT * FROM cld.event where calendar_id = ?";
          stm = conn.prepareStatement(sql);
          stm.setString(1, request.getParameter("idcalendar"));
...
<body>
  ...
  <div id="cnameDisplay" style="width: 76%;margin: auto;text-align:center;font-size: 45px;padding: 30px;border-bottom: 1px solid black;background-color: #ffe6cc;">
    <b><%=request.getParameter("cname")%></b>
      <div style="position: absolute;top: 80px;right: 170px;">
          <span onclick="editCalendarName()" class="material-icons" style="font-size: 30px;cursor: pointer;">
            edit
          </span>
          <span onclick="document.getElementById('deleteCalendarConfirm').style.display='block'"
                class="material-icons" style="font-size: 30px;cursor: pointer;">
            delete
          </span>
          <!-- Delete Calendar Confirm Form -->
          <div id="deleteCalendarConfirm" class="backContainer" style="text-align: left;">
            <form class="confirm-form" action="DeleteCalendarServlet" method="post">
              <input type="text" name="idcalendar" value="<%=request.getParameter("idcalendar")%>" hidden>
              <div class="confirm-header">
                Calendar Deletion <span style="float:right;font-size: 22px;cursor: pointer;margin-top: -5px;" onclick="document.getElementById('deleteCalendarConfirm').style.display='none'">&times;</span>
              </div>
              <div class="confirm-text" id="confirm-text">
                Are you sure you wish to delete <b><%=request.getParameter("cname")%></b> ?
              </div>
              <div class="confirm-form-but">
                <button type="submit" name="button">Confirm</button>
                <button type="button" onclick="document.getElementById('deleteCalendarConfirm').style.display='none'">Cancel</button>
              </div>
            </form>
          </div>
  ...
  <div id="cnameInput" style="display: none;width: 76%;margin: auto;text-align:center;font-size: 45px;padding: 30px;border-bottom: 1px solid black;background-color: #ffe6cc;">
      <form action="EditCalendarNameServlet" method="post">
      <input type="text" name="calendarName" value="<%=request.getParameter("cname")%>"
             style="width: 85%;text-align: center;padding: 5px;border: none;outline: none;
             border-bottom: 2px solid #ddd;font-size: 45px;">
      <input type="text" name="idcalendar" value="<%=request.getParameter("idcalendar")%>" hidden>
      <div style="position: absolute;top: 80px;right: 170px;">
          <span onclick="document.getElementById('submitEditCname').click()"
                class="material-icons" style="font-size: 30px;cursor: pointer;">
            save
          </span>
          <span onclick="cancelEditCalendarName()" class="material-icons" style="font-size: 30px;cursor: pointer;">
            cancel
          </span>
      </div>
      <input id="submitEditCname" type="submit" hidden>
      </form>
  </div>
  <div class="container">
        ...
        <form action="AddEventServlet" onsubmit="return validateEvent()" method="post">
          ...
          <div class="eventFormContent">
            ...
            <select id="colorSelect" name="color" onchange="changeDpColor()">
              <option value="#007acc" selected>Blue</option>
...
<script>
    function editCalendarName() {
        document.getElementById('cnameDisplay').style.display = 'none';
        document.getElementById('cnameInput').style.display = 'block';
    }

    function cancelEditCalendarName() {
        document.getElementById('cnameDisplay').style.display = 'block';
        document.getElementById('cnameInput').style.display = 'none';
    }
